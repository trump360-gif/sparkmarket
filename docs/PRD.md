...
| 1.4 | 2025-12-13 | Phase 10 완료 - 가격 빠른 수정 및 판매 알림 기능 |
...
- [x] **가격 빠른 수정 기능**
  - `PATCH /products/:id/price` API 추가 (가격만 변경하는 전용 엔드포인트)
  - 마이페이지 상품 카드에 연필 아이콘 추가
  - 상품 상세 페이지에 가격 수정 버튼 추가 (판매자 본인만 표시)
  - 가격 수정 모달에 빠른 가격 인하 버튼 추가 (-1천, -5천, -1만, -5만, -10만)
  - 가격 인하 시 찜한 사용자에게 알림 자동 발송
- [x] **판매 완료 알림 기능**
  - 상품 구매 시 판매자에게 알림 전송 (`PRODUCT_SOLD`)
  - 알림 내용: 상품명, 구매자 닉네임, 판매 금액 포함
- [x] **브랜드 ID 검증 강화**
  - 상품 등록/수정 시 유효하지 않은 brand_id 검증 추가
